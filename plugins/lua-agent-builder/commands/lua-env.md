...
- "Environment?" (options: `sandbox` — writes the project's local `.env`, the file `lua test` reads; `production` — the live agent's variables. `staging` is the CLI's alias for `sandbox`)
...
- After a `sandbox` set, `lua test` / `lua test workflow` pick the new value up on their next run (they merge `.env` over `process.env`). After a `production` set, new invocations see it at once; the CLI hints a production chat to verify — offer `/lua-chat` (production) instead of running it here.
