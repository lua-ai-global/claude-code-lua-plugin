...
33. ⏳ lua-cli > 3.35.0: an agent step's `taskClass` needs a `model` on a pushed definition (`task-class-without-model`, server-side at push — `lua compile` passes it); `effort` is recorded and estimated but reaches the model only after `lua push workflow --apply-effort`; the agent-level `modelSettings.reasoning.effort` scale (`off / minimal / max`) is a different thing (workflows.md §2).
34. ⏳ lua-cli > 3.35.0: `lua push all` will push every workflow **and activate the pushed version** (main `push.ts`, PR #3024) — a production change the 3.35.0 stage-all does not make (cli-reference.md §4).
