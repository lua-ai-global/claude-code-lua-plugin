...
16. **Typings run ahead of the deployed runtime** (3.33.0): `Data.create`/`Data.update` with an options object → `400 searchText must be a string` in production (use the string/2-arg forms); `Data.collections()` is not injected deployed; `Voice.createSession` is not injected deployed **or** under `lua test`; `Workflows.raiseBudget` fails in both (use `lua workflows raise-budget`); `LuaMCPServerConfig` has no `description`.
