...
Two shipped platform features reach the knowledge base, the slash commands and the subagents: **per-step model classes** for workflows and the **workflow autonomy envelope** (pre-consented starts). Every claim was read from lua-core-services — lua-cli `main` at `12cefb7ec` (WMC-E7 `#2969`, hotfixes H7 `#3053` and H8 `#3051`, the stage-all fix `#3024`) and `feat/workflow-autonomy` at `378403322` (WMC-A1…A7) — never from the public docs. **Version decision**: the newest published lua-cli is 3.35.0 (tag `lua-cli@3.35.0` = `8d65d1ba8`, 2026-09-15, npm 14:13 UTC) and carries none of it; `packages/lua-cli/package.json` on both refs still says 3.35.0 and no release PR is open, so both features are described as "⏳ requires lua-cli > 3.35.0 (unreleased as of 2026-09-18)" and `PINNED_MIN_LUA_CLI` stays 3.33.0. No hook behaviour changed (no hook reads a `lua workflows` exit code); the permission template gains the new verbs.
...
- ⚠ Forward-looking: lua-cli main `push.ts` (`#3024`, `af66332ee`) adds workflows to stage-all and **always activates** the pushed version — on the next lua-cli `lua push all` is a workflow go-live. `cli-reference.md` §4/§5, `primitives.md` §14 (34), `/lua-push`, `/lua-deploy` and the deploy pilot say so; the hook gate is unchanged (open question in the audit report).
...
- Bump 1.2.2 → 1.3.0 everywhere; `mcp/lua-platform/dist/server.js` rebuilt.
