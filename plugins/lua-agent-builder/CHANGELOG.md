...
- No hook behaviour changes beyond the pin: `hooks/hooks.json`, `confirm-deploy`, `post-deploy-smoke` and `lib/tokenizer.mjs` are untouched, because a log drain is organization configuration and not a production verb.
