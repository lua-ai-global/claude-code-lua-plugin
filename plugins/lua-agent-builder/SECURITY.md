...
- **MCP servers** — `mcp/lua-platform/dist/server.js`, a local stdio server exposing 5 read-only tools (a single self-contained bundle since 1.6.0 — its npm dependencies, `@modelcontextprotocol/sdk` included, are inlined at build time from the lockfile); it talks to `https://api.heylua.ai` over HTTPS with the user's lua-cli credential (and, for a session login, refreshes the stored refresh token at Google's securetoken endpoint exactly like lua-cli does). `lua-docs` is the public remote MCP at `https://docs.heylua.ai/mcp` (read-only docs search).
...
|---|---|
| Production gate: `lua deploy`, `lua skills\|webhooks\|jobs\|preprocessors\|postprocessors deploy`, `lua persona production deploy`, `lua workflows deploy\|activate`, `lua version promote`, `lua mcp activate`, `lua marketplace template publish\|apply` are blocked in bare form; only the `LUA_DEPLOY_CONFIRMED=1`-prefixed form emitted after the user's confirmation runs. The gate covers every spelling lua-cli accepts: the action aliases from its `aliases.ts` (`publish`→deploy, `on`/`enable`→activate, `submit`/`publish_version`→template publish, `deploy`/`fleet-apply`/`rollout`→template apply, `prod`/`prd`/`live`→production) and all three installed binaries (`lua`, `heylua`, `lua-ai`) | **The hook is the gate.** `hooks/confirm-deploy.mjs` runs on **every** Bash call (no `if` glob to drift), classifies with `lib/tokenizer.mjs` (the single source of truth) and exits 2 on a bare verb — a hook block takes precedence over any allow rule, including a broad `Bash(lua *)` in the user's own settings. Since 1.6.0 the classifier lexes the whole command. It recognises the binary in command position: after assignments, and as the command of a wrapper, launcher or keyword (`sudo`, `env`, `timeout`, `xargs`, `npx lua`, `npx lua-cli`, `pnpm exec lua`, `node …/lua-cli/…`, `find -exec`, `if`/`then`), at any path and in any case. That applies to every simple command of a `&&`/`;`/`||`/pipe chain, to `( )`/`{ }` groups, to `$( )`/backticks/`<( )`, and to strings run as code: `bash -c`, `eval`, `ssh`, `env -S`, `… | sh`, heredocs fed to a shell or written to a script that is then run, inline interpreter code, awk `system()`, sed `e`, and `git -c alias.x='!…'`. A word the shell computes at runtime (`$VAR`, `$(…)`, `${IFS}`, globs, brace expansion) in the binary or verb position is blocked. The prefix counts only as the first word of the very simple command that runs the verb, with the binary spelled bare. It never counts through a pipe, group, substitution, wrapper string or launcher, and each verb in a chain needs its own. The classifier is linear and **fails closed**: a command over 32 KB, one that exhausts the 1.5 s budget, or an internal error blocks the command if it mentions lua. This matters because a hook that hits its 10 s timeout lets the command run; `test/hooks/confirm-deploy.timeout.test.mjs` spawns the real hook on 100 KB adversarial inputs. With `LUA_PLUGIN_HEADLESS=1` the prefix is void and every production verb is blocked (docs/JOB_TIER.md). The hook is still a belt against a model, not a sandbox: a command assembled at runtime (`base64 -d \| sh`, a script file, an npm script, a raw HTTP call) is out of its reach — in unattended runs the Lua-API proxy is the boundary. `lib/permissions-template.json` allows the literal prefixed forms (so the confirmed command runs without a second prompt) and deliberately has **no** deny/ask rule for the bare verbs: Claude Code evaluates deny/ask rules past a leading env assignment (code.claude.com/docs/en/permissions: "A deny or ask rule matches past any leading assignment"), so a `Bash(lua deploy*)` deny would also block the confirmed form and make every deploy impossible — verified live on 2026-09-12. A bare verb that reaches the permission layer falls to Claude Code's default prompt (a denial in `-p` mode). `heylua`/`lua-ai` are denied wholesale. `test/lib/permissions-mirror.test.mjs` and `scripts/lint-permissions.mjs` fail if a deny/ask rule would shadow a confirmed form or an allow rule admits a bare one |
...
| Post-deploy smoke check fires after every verb that makes something live (`SMOKE_LABELS` in `lib/tokenizer.mjs`: deploy spellings, `persona production deploy`, `workflows deploy`, `version promote`, `mcp activate`); never in headless mode, which sends no production ping | `hooks/post-deploy-smoke.mjs` (PostToolUse on every Bash call) |
| Headless mode (`LUA_PLUGIN_HEADLESS=1`): no hook message points at a slash command (they need AskUserQuestion or the Agent tool), and nothing blocked interactively is allowed | `lib/headless.mjs`; `test/hooks/headless.test.mjs` |
