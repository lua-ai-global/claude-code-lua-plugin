...
- `<name>` — trigger: <schedule | trigger | tool | chat>; steps: <step1 → agentStep (taskClass, class/<c>, requires, effort) → approval (approver, timeout, onTimeout) → foreach … → step>; budget <credits>; Job tier? <why>; consent: <auto — within 15 steps · 20 credits · 1 h | asks a person (why) | needs the org autonomy envelope: `lua workflows policy autonomy set …`> (⏳ lua-cli 3.36.0 or later for the model members and the policy verbs)
